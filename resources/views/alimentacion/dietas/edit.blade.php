...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-edit text-success"></i> Editar Dieta
        </h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('alimentacion.dietas.update', $dieta['id']) }}" id="dietaForm">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="finca_id" name="finca_id" required>
                            ...
                            <option value="{{ $finca['id'] }}" {{ old('finca_id', $dieta['finca_id']) == $finca['id'] ? 'selected' : '' }}>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="tipo_animal" name="tipo_animal" required>
                            ...
                            <option value="bovino" {{ old('tipo_animal', $dieta['tipo_animal']) == 'bovino' ? 'selected' : '' }}>Bovino</option>
                            <option value="porcino" {{ old('tipo_animal', $dieta['tipo_animal']) == 'porcino' ? 'selected' : '' }}>Porcino</option>
                            <option value="caprino" {{ old('tipo_animal', $dieta['tipo_animal']) == 'caprino' ? 'selected' : '' }}>Caprino</option>
                            <option value="ovina" {{ old('tipo_animal', $dieta['tipo_animal']) == 'ovina' ? 'selected' : '' }}>Ovina</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="categoria" name="categoria" required>
                            ...
                            <option value="ternero" {{ old('categoria', $dieta['categoria']) == 'ternero' ? 'selected' : '' }}>Ternero</option>
                            <option value="desarrollo" {{ old('categoria', $dieta['categoria']) == 'desarrollo' ? 'selected' : '' }}>Desarrollo</option>
                            <option value="adulto" {{ old('categoria', $dieta['categoria']) == 'adulto' ? 'selected' : '' }}>Adulto</option>
                            <option value="lactancia" {{ old('categoria', $dieta['categoria']) == 'lactancia' ? 'selected' : '' }}>Lactancia</option>
                            <option value="gestacion" {{ old('categoria', $dieta['categoria']) == 'gestacion' ? 'selected' : '' }}>Gestación</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" 
                              ...
                              rows="3" 
                              placeholder="Descripción de la dieta, objetivos nutricionales, etc.">{{ old('descripcion', $dieta['descripcion']) }}</textarea>
                </div>

                <!-- Estado Activa -->
                <div class="mb-3">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="activa" name="activa" value="1" {{ old('activa', $dieta['activa']) ? 'checked' : '' }}>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button type="submit" class="btn btn-success">
                        <i class="fas fa-save me-2"></i>Actualizar Dieta
                    </button>
                </div>
            </form>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Agregar alimento
        function addAlimento(alimentoData = null) {
            const template = alimentoTemplate.innerHTML.replace(/INDEX/g, alimentoCount);
            const div = document.createElement('div');
            div.innerHTML = template;
            alimentosContainer.appendChild(div);
            
            // Si hay datos del alimento, llenar los campos
            if (alimentoData) {
                const alimentoSelect = div.querySelector('.alimento-select');
                const cantidadInput = div.querySelector('.cantidad-input');
                const frecuenciaInput = div.querySelector('input[name*="frecuencia"]');
                const observacionesInput = div.querySelector('input[name*="observaciones"]');
                const unidadText = div.querySelector('.unidad-text');
                
                // Seleccionar alimento
                if (alimentoData.alimento_id) {
                    alimentoSelect.value = alimentoData.alimento_id;
                ...
                // Llenar otros campos
                if (alimentoData.cantidad) cantidadInput.value = alimentoData.cantidad;
                if (alimentoData.frecuencia) frecuenciaInput.value = alimentoData.frecuencia;
                if (alimentoData.observaciones) observacionesInput.value = alimentoData.observaciones;
            }
        ...
        // Event listeners
        addAlimentoBtn.addEventListener('click', function() {
            addAlimento();
        });
        ...
        // Cargar alimentos existentes de la dieta
        @if(isset($dieta['alimentos']) && count($dieta['alimentos']) > 0)
            ...
            addAlimento({
                ...
                frecuencia: "{{ $alimento['pivot']['frecuencia'] ?? $alimento['frecuencia'] ?? '' }}",
                observaciones: "{{ $alimento['pivot']['observaciones'] ?? $alimento['observaciones'] ?? '' }}"
            });
            @endforeach
        @else
            // Si no hay alimentos, agregar uno vacío
            addAlimento();
        @endif
